...
1. **3 open HIGH gaps** — the remaining high-value scrml-correctness work (delicate BS/codegen, fresh-arc-shaped per the repeated-crash + S140 PA-direct-during-instability lessons):
   - ~~`g-nested-component-member-arg-misparse`~~ — **RESOLVED post-wrap (PA-direct).** The member-access arg `<Badge s=row.name/>` / `status=load.status` was space-padded by the logic tokenizer (`row . name`) + not collapsed by CE; fixed with a general member-access collapse in `component-expander.ts` (mirrors the call-form + `@.` collapses). R26 on the real board: `statusLabel(load.status)` (member preserved). Its case-c (inlined-root class `${…}` interp raw) SPLIT to `g-inlined-component-root-class-interp-raw` (MED) — see below.
   - `g-inlined-component-root-class-interp-raw` (**MED**, NEW S200 post-wrap, split from the above) — an INLINED component's ROOT class-attr `${…}` interp emits raw (`setAttribute("class", "base ${cls(status)}")`; valid JS, silent-wrong). NOT a board blocker (the board's badge is module-imported, not inlined). Repro `/tmp/g-nested-arg/v6-classinterp.scrml`. Needs a CE inlined-root class-attr lowering review.
