...
- **Last-absorbed delta seq:** **S210 [7]** (T124 absorbed [6] AE ruling (a)→(b) REVERSAL [PA Rule-4 MISS self-corrected: "reject name= on `<engine>`" → re-dispatched form (b); informational] · [7] ss13 NO-EXECUTE re-integration). T123 absorbed S210 [1] COLD boot [Profile A FULL; **digest STALE c2f8f1fd → authoritative fallback** — see ROUTE-TO-PA below] · [2] 3 HIGH triaged [AD g-attr-interp-fn-name-not-renamed · AE · AF, all R26-verified] · [3] 2 HIGH fix dispatches · [4] 3 sibling acks + giti LOW filed · [5] sPA ss4 block-splitter-native-parser RE-INTEGRATED). All informational/contract — NO maintenance-shaped `(vpa:)`. (Prior: S209 boot [10]-[18]; [19]-[31].)
...
- **Owed maintenance: MAPS DUE-but-DEFERRED (≥2 STRUCTURAL mapped changes — real refresh needed).** S210 landing `8c938a58` changed 5 mapped-src files: `ast-builder.js` (+75), `code-segments.ts` (+151), `engine-statechild-parser.ts` (+33), `expression-parser.ts` (+24), `tokenizer.ts` (+8). ≥2 + SUBSTANTIAL (new parser/codegen logic — NOT a watermark-bump-trivial change like T113/T120; needs a real **project-mapper run**). the S210 PA is mid-burst (agent `a1ad1907` AE-b in flight) → DEFER to burst-settle to avoid re-run. T124: **digest regen'd current @ `3f3fb5c6` seq 7** (deltas [6-7] moved delta-log). §0 PASS, §3c PASS (no drift). (Watermark still `41422726`; the 5 structural files remain owed.)
...
- **heartbeat:** tick **T124** · last-absorbed **[S210 7]** ([6-7] absorbed; digest regen'd `3f3fb5c6` seq 7) · deputy-maint rebased onto main `956460af` (ss13-reconcile SPEC-INDEX §59 flip). F3: ss13 LANDED [7]; `a1ad1907` = AE-b re-dispatch (engine-name-dual-table) in flight. main fast-moving (S210 active).
...
- **S210 PA dispatch `agent-a1ad1907`** @`faa213c5` ("engine-name-dual-table" = the AE re-dispatch form (b) per [6]) — actively working. The S209-wrap bug-fix agents all LANDED in `8c938a58` + cleaned.
- **sPA `ss4` LANDED** ([5] `8c938a58`). **sPA `ss13` LANDED** ([7] `c3e9d16e`+reconcile `956460af`, NO-EXECUTE; worktree `../scrml-spa-ss13` still present → PA 6b-cleanup pending).
...
## Currency snapshot (@ tick 124 — S210 active)

- **maps:** watermark **`41422726`** — **DUE-but-DEFERRED** (5 structural mapped-src files in `8c938a58`; real project-mapper run when the S210 burst settles — `a1ad1907` AE-b still in flight). **digest:** current (regen'd @ `3f3fb5c6`, seq 7). **§0:** PASS. **§3c:** PASS (450n/168e). board **HIGH 1 · MED 11 · LOW 17 · Nom 8**. main `956460af`. **(ROUTE-TO-PA wrap-digest miss still open — see Last-absorbed block.)** **§0:** gap-counts + recent-sessions PASS. **§3c:** PASS (445n/168e). board **HIGH 0 · MED 11 · LOW 18 · Nom 8** (ground-truth oracle @ HEAD; ss2 reconcile −1 MED).
