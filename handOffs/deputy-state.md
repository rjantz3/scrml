...
- **Last-absorbed delta seq:** **S210 [17]** (T129 absorbed [13] dpa-001 A2 RATIFIED [user "ratify ship A2" — ship the thin declared-shape `<api>` primitive] · [14] 6nz provenance reply sent · [15] bookkeeping [user-voice S210 + changelog + inbox→read/] · [16] A2 build SCOPED [`docs/changes/api-primitive-a2-2026-06-20/SCOPE-AND-DECOMPOSITION.md`] · [17] 6nz AF surfaced [§36.6 input-state-read-at-animation-moment ruling]). All informational/design — NO maintenance-shaped `(vpa:)`. T127: [12] dpa-001 drained. T125 absorbed [8] list-rebuild launch + fattening doctrine · [9] AE dual-table LANDED [HIGH 1→0] · [10] dPA MV BUILT + backend debate banked. T124 absorbed [6] AE ruling (a)→(b) reversal · [7] ss13 NO-EXECUTE. T123 absorbed S210 [1] COLD boot [Profile A FULL; **digest STALE c2f8f1fd → authoritative fallback** — see ROUTE-TO-PA below] · [2] 3 HIGH triaged [AD g-attr-interp-fn-name-not-renamed · AE · AF, all R26-verified] · [3] 2 HIGH fix dispatches · [4] 3 sibling acks + giti LOW filed · [5] sPA ss4 block-splitter-native-parser RE-INTEGRATED). All informational/contract — NO maintenance-shaped `(vpa:)`. (Prior: S209 boot [10]-[18]; [19]-[31].)
...
- **heartbeat:** tick **T129** · last-absorbed **[S210 17]** ([13-17] absorbed; digest regen'd `c1c96ca1` seq 17) · deputy-maint @`c1c96ca1`+ (T128 no-op). F3: `spa/ss3` IN FLIGHT (item3 landed on branch). **WATCH: A2 `<api>` primitive build ratified+scoped → its compiler/src landing triggers a maps refresh.** main `c1c96ca1`.
...
- **sPA `ss3` IN FLIGHT** — worktree `../scrml-spa-ss3` @`8676687b` (dispositioning: item3 landed-on-branch + progress log; no re-int msg yet). Watch — if it lands compiler/src on merge, queue a maps refresh per cadence.
- **WATCH — A2 `<api>` primitive BUILD coming:** ratified [13] + scoped [16] (`api-primitive-a2-2026-06-20/SCOPE-AND-DECOMPOSITION.md`). When the PA dispatches + lands it, it WILL touch compiler/src → maps refresh trigger. The structural-refresh shape (T125) applies.
- **DONE/cleaned:** AE-b/ss4/ss13 landed+cleaned; list-rebuild [11] + dPA dpa-001 [12]→A2-ratified [13] complete; 7 inbox msgs → read/ ([15] bookkeeping).
- **(maps consumer note):** T125 structural refresh in main `a3a7e091` BEFORE the ss3 wave → fresh dev-agents navigate current maps.
...
## Currency snapshot (@ tick 129 — S210 active)

- **maps:** watermark **`5c68e87e`** — CURRENT ([13-17] doc/design burst, no mapped src). **digest:** current (`c1c96ca1`, seq 17). **§0:** PASS. **§3c:** PASS (450n/168e). board **HIGH 0 · MED 11 · LOW 17 · Nom 8**. main `c1c96ca1`. **WATCH: A2 `<api>` build coming (compiler/src → maps refresh). (ROUTE-TO-PA wrap-digest miss still open.)** **§0:** gap-counts + recent-sessions PASS. **§3c:** PASS (445n/168e). board **HIGH 0 · MED 11 · LOW 18 · Nom 8** (ground-truth oracle @ HEAD; ss2 reconcile −1 MED).
