// scrml:compiler/ts — runtime shim (DEFERRED)
//
// Per-stage sibling of compiler/runtime/stdlib/compiler.js (the umbrella).
// Mirrors stdlib/compiler/ts.scrml's export surface; every export is a thunk
...
function _unavailable(name) {
  throw new Error(
    `[scrml:compiler/ts] ${name}() is not available at runtime via the scrml:compiler/ts shim. `
      ...
      + `the compiler-source path: import { runTS } from "<...>/compiler/src/type-system.ts".`,
  );
}

export const runTS = (...args) => _unavailable("runTS");
export const TSError = (...args) => _unavailable("TSError");
