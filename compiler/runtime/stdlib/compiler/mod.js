// scrml:compiler/mod — runtime shim (DEFERRED)
//
// Per-stage sibling of compiler/runtime/stdlib/compiler.js (the umbrella).
// Mirrors stdlib/compiler/mod.scrml's export surface; every export is a thunk
...
function _unavailable(name) {
  throw new Error(
    `[scrml:compiler/mod] ${name}() is not available at runtime via the scrml:compiler/mod shim. `
      ...
      + `the compiler-source path: import { resolveModules } from "<...>/compiler/src/module-resolver.js".`,
  );
}
