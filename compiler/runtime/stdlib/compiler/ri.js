// scrml:compiler/ri — runtime shim (DEFERRED)
//
// Per-stage sibling of compiler/runtime/stdlib/compiler.js (the umbrella).
// Mirrors stdlib/compiler/ri.scrml's export surface; every export is a thunk
...
function _unavailable(name) {
  throw new Error(
    `[scrml:compiler/ri] ${name}() is not available at runtime via the scrml:compiler/ri shim. `
      ...
      + `the compiler-source path: import { runRI } from "<...>/compiler/src/route-inference.ts".`,
  );
}

export const runRI = (...args) => _unavailable("runRI");
export const RIError = (...args) => _unavailable("RIError");
export const collectFileFunctions = (...args) => _unavailable("collectFileFunctions");
export const generateRouteName = (...args) => _unavailable("generateRouteName");
export const buildFunctionIndex = (...args) => _unavailable("buildFunctionIndex");
export const buildPageRouteTree = (...args) => _unavailable("buildPageRouteTree");
