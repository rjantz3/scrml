// scrml:compiler/me — runtime shim (DEFERRED)
//
// Per-stage sibling of compiler/runtime/stdlib/compiler.js (the umbrella).
// Mirrors stdlib/compiler/me.scrml's export surface; every export is a thunk
...
function _unavailable(name) {
  throw new Error(
    `[scrml:compiler/me] ${name}() is not available at runtime via the scrml:compiler/me shim. `
      ...
      + `the compiler-source path: import { runMetaEval } from "<...>/compiler/src/meta-eval.ts".`,
  );
}

export const runMetaEval = (...args) => _unavailable("runMetaEval");
export const MetaEvalError = (...args) => _unavailable("MetaEvalError");
