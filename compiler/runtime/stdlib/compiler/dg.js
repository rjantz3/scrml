// scrml:compiler/dg — runtime shim (DEFERRED)
//
// Per-stage sibling of compiler/runtime/stdlib/compiler.js (the umbrella).
// Mirrors stdlib/compiler/dg.scrml's export surface; every export is a thunk
...
function _unavailable(name) {
  throw new Error(
    `[scrml:compiler/dg] ${name}() is not available at runtime via the scrml:compiler/dg shim. `
      ...
      + `the compiler-source path: import { runDG } from "<...>/compiler/src/dependency-graph.ts".`,
  );
}

export const runDG = (...args) => _unavailable("runDG");
export const DGError = (...args) => _unavailable("DGError");
