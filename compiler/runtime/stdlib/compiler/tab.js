// scrml:compiler/tab — runtime shim (DEFERRED)
//
// Per-stage sibling of compiler/runtime/stdlib/compiler.js (the umbrella).
// Mirrors stdlib/compiler/tab.scrml's export surface; every export is a thunk
...
function _unavailable(name) {
  throw new Error(
    `[scrml:compiler/tab] ${name}() is not available at runtime via the scrml:compiler/tab shim. `
      ...
      + `the compiler-source path: import { buildAST } from "<...>/compiler/src/ast-builder.js".`,
  );
}

export const buildAST = (...args) => _unavailable("buildAST");
export const runTAB = (...args) => _unavailable("runTAB");
export const parseLogicBody = (...args) => _unavailable("parseLogicBody");
export const TABError = (...args) => _unavailable("TABError");
