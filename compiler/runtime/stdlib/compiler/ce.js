// scrml:compiler/ce — runtime shim (DEFERRED)
//
// Per-stage sibling of compiler/runtime/stdlib/compiler.js (the umbrella).
// Mirrors stdlib/compiler/ce.scrml's export surface; every export is a thunk
...
function _unavailable(name) {
  throw new Error(
    `[scrml:compiler/ce] ${name}() is not available at runtime via the scrml:compiler/ce shim. `
      ...
      + `the compiler-source path: import { runCE } from "<...>/compiler/src/component-expander.ts".`,
  );
}

export const runCE = (...args) => _unavailable("runCE");
export const runCEFile = (...args) => _unavailable("runCEFile");
