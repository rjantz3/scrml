...
4. **native attr-value `exprNode` population — DISPATCHING NEXT (LARGE, cross-cutting ~162 files).** native `tag-frame.js` (~L1079/1095/1125/1130/1153) builds attr values `{kind:"expr"|"variable-ref",raw,refs,sourceText,span}` WITHOUT `exprNode`; live `ast-builder.js:1834/1857/1878/2217` sets it via `safeParseExprToNodeGlobal`. `emit-html.ts` reads `val.exprNode` (handlers @1735 / if= @1718 / bindings @1756 / body @1015) → absent on native → raw `@` → `E-CODEGEN-INVALID-JS`. Likely kills the `E-CODEGEN-INVALID-JS` (18) + handler-cluster of the 674. **Verify on its own fixture (any `onclick=@x.method()`), then re-verify the message-dispatch full-fixture R26 closes with both fixes present.**

### SYNC NOTE — origin behind local (push held)
- **scrmlTS local HEAD is AHEAD of origin/main by 2+ PA commits (`154a1799` session-start, `7cbad5dd` message-arm, + this maps/hand-off commit) — NOT pushed** (user said "land, not push"). **Consequence:** `isolation:worktree` dispatches branch from origin/main (`f11db672`), so the exprNode worktree's startup MUST `git merge <latest-local-HEAD>` to inherit the message-arm landing + fresh maps (briefed). If/when push is authorized, this gymnastics goes away.
