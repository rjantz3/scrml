...
Size:       ~1400 source files (914 test .test.js by runner + 145 compiler/src [112 .ts + 33 .js] + 37 native-parser .js + stdlib + lsp)
...
| test.map.md          | present | bun:test, 914 .test.js files (23,075 pass / 0 fail at S167, within-node 1005/0); +5 S162; +4 S164 native-parser unit (native-attrvalue-exprnode-population, native-lift-markup-closetag-span, native-sql-chained-form-f2a, native-tablefor-struct-field-drop) + within-node/conformance updates S164-S165; +1 S166 cross-file-export emitted-output §B regression (m6.4a-native-p2-form1.test.js); S166 bare-`function` failable within-node residual-rebump (14 fixtures, 1005→991→1005); **+2 S167 deep-set write-loss (deepset-write-loss-position.test.js 16/87 emit-shape + browser-deepset-write-loss.test.js 4 happy-dom runtime); +4 within-node native-lag allowlist bump on r11-elixir-chat** |
