...
| codegen/emit-match.ts | (match arm emission); **S213 render-bridge: `resolveOnExpr` imports `collectRequestIds` + threads `requestIds` into `emitExpr` call for the `on=` expr** |
