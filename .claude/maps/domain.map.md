...
| A-4 per-route artifact splitter | A-4.1 + A-4.2 + A-4.3 + A-4.5 + A-4.6 closed S91 — `compiler/src/codegen/route-splitter.ts` (orchestrator + initial-chunk + tier-1 composers + content-address hash) + `compiler/src/codegen/atom-emitter.ts` (per-id atom helpers) + NEW `compiler/src/codegen/fnv1a-hash.ts` (shared FNV-1a primitive) + `_scrml_prefetch_tier1` + `_scrml_fetch_chunk` runtime in `compiler/src/runtime-template.js`. Opt-in via `--emit-per-route` CLI flag. A-4.4/A-4.7 pending. Test anchors: `compiler/tests/unit/codegen-route-splitter.test.js` (43 tests), `compiler/tests/unit/codegen-route-splitter-tier-n.test.js` (14 tests — A-4.5 runtime + tree-shake), `compiler/tests/unit/chunk-content-addressing.test.js` (19 tests — A-4.6 hash format + determinism + no-placeholder-leak + §47.1.3 parameter conformance), `compiler/tests/integration/initial-chunk-emission.test.js` (20 tests — §40.9.9 replay + A-4.6 5-run hash determinism + source-change-flips-hash), `compiler/tests/integration/tier1-idle-prefetch.test.js` (9 tests — §40.9.9 empty-tier-1 normative + tree-shake DEAD verification) |
