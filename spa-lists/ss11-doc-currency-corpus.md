...
3. **`r28-c2`** `[parked-escalated-PA]` bug MED · tier low — R28-C2 canon residuals: `< db>`/`< schema>` leading-space + `print()` residual. Entry: docs/articles/llm-kickstarter-v2-2026-05-04.md. **NOT a residual cleanup — escalated.** R26 probe: despace direction is empirically correct (no-space `<db>` canonical, spaced `< db>` fires W-WHITESPACE-001 → E-WHITESPACE-001@P3), BUT SPEC §4 (L367/599/27198/31031, EBNF L8990) still teaches space-REQUIRED + no-space=HTML — a SPEC self-contradiction needing a ruling + amendment (PA-only). Blast radius corpus-wide (SPEC=194 spaced openers, examples=27, articles=36). `print()` = 0 hits (moot). See progress.md.
