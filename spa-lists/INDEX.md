...
| 17 | each-codegen | 3 | emit-each · emit-html | **IN-FLIGHT S219** (`spa/ss17`; dispatched combined-fix agent). 3 MED each-codegen (g-each-peritem-markup-value-ternary · g-nested-each-outer-key-reuse-inner-frozen · g-expr-event-handler-dead-in-each). **flogence #1/#3 + ss19-GroupB are the same family → slot here as a follow-on once landed.** | full |
