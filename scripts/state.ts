...
function digest(): string {
  ...
  L.push("");
  L.push(`> ⚠ **FRESHNESS GUARD (PA — read this first).** This digest is a mechanical projection`);
  L.push(`> reflecting **HEAD \`${head}\`** + delta-log **[${seq}]**. Before trusting it, run`);
  L.push(`> \`git rev-parse --short HEAD\`. **If it ≠ \`${head}\`, DISTRUST this digest** and fall back to the`);
  L.push(`> authoritative reads (master-list §0 + hand-off.md + delta-log tail). Every line below is`);
  L.push(`> \`asserted\` (a projection), never \`verified\` — confirm load-bearing claims against source.`);
  L.push(`> **A drifted digest is worse than reading cold.**`);
  L.push(`>`);
...
// Digest freshness (WARN-only in --check + the no-arg report). NOT gated: the digest naturally lags
// HEAD by the deputy's regen cadence; freshness is enforced at the PA-start guard, not the currency gate.
function digestStaleness() {
  let text = "";
  try { text = readFileSync(`${ROOT}/handOffs/digest.md`, "utf8"); } catch { return { note: "digest: not generated yet" }; }
  const m = text.match(/<!--\s*@digest\s+head=([0-9a-f]+)\s+delta-seq=(\S+?)\s*-->/i);
  const head = sh("git", ["rev-parse", "--short", "HEAD"]).stdout.trim();
  if (!m) return { note: "digest: header not parseable" };
  if (m[1] === head) return { note: `digest: current (head ${m[1]}, delta-seq ${m[2]})` };
  return { note: `digest: STALE — reflects head ${m[1]} (delta-seq ${m[2]}), HEAD is ${head} → PA distrusts + falls back` };
}
