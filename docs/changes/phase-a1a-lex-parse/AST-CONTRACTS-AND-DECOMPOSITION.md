...
| 8 | E-RESERVED-IDENTIFIER trigger | `ast-builder.js` only — emit error when `function` / `fn` declarator uses `reset` as the name | 0.5 h | ~2 trigger tests + ~2 negative-guard tests |
