...
- 2026-06-10 S180: **D4 SPLIT into D4a (examples) + D4b (samples) + D4c (docs).** D4a + D4c dispatched in parallel @ base `e1d4f88c` (file-disjoint).
- 2026-06-11 S180: **D4c LANDED + PUSHED `b01479e4`** (docs: 65 SPEC + 18 kickstarter + 4 PRIMER worked-example migrations; computeDouble/redeem→server fn; teaching/SSE/server-fn/session-only LEFT; 6 spot-compiles server-boundary; SPEC-INDEX regen). D5 follow-on surfaced: compiler ERROR-OUTPUT suggestions still say `server function` (saveOrder/logEdit/getUser — code+spec change).
- 2026-06-11 S180: **D4a LANDED + PUSHED `<pending>`** (examples → 0 non-SSE `server function` decls; 2 real bodies added [mintTicket/submit]; 5 channel strips via T7; 5 hand-strips [SQL-lift + handle, Migration 4 blocked]; 14-mario arm-arrow co-canon; full suite 23809/0). **VERIFIED.md re-verification flagged** (examples changed).
- 2026-06-11 S180: **NEW D3.1 INSERTED before D4b — fix 2 Migration-4 tool gaps D4a surfaced:**
  - **Gap A (route-inference.ts:3172):** W-DEPRECATED-SERVER-MODIFIER SUPPRESSED on `lift`-bearing fns via `!hasLiftInFunctionBody` — a stale S93 safety (lift was illegal in plain `function`). D1 made lift-as-return valid in INFERRED-server fns (§10.4 keyed on boundary), so dropping `server` from a lift-bearing escalating fn is now SAFE → the suppression must go (fire W-DEPRECATED on lift-bearing fns that have another escalation reason). **D1 sibling-miss.** Without it Migration 4 skips the whole SQL-lift class (huge in samples).
  - **Gap B:** handle's W-DEPRECATED span starts 2 bytes into `server` (`rver function handle`) → Migration 4 `readWordAt(span.start)` reads `rver` + skips. **D2 span bug** — fix the middleware-handle reason's span to anchor at `server` (like channel publishers).
  - D3.1 = route-inference.ts fix both + tests (Migration 4 strips a lift-SQL `server function` + a `handle` post-fix) + R26. THEN D4b.
- Pre-D4 baseline: examples 24, samples 221 non-SSE `server function`. Dog-food confirmed T7 on a fresh keyless channel publisher.
- 2026-06-11 S180: **D4b (samples) RESOLVED as a documented NO-OP (user-ruled "leave samples, go to close").** `migrate` DELIBERATELY excludes `samples/`+`tests/` (migrate.js:2400 — they exercise deprecation paths on purpose); gauntlet samples don't compile standalone (DB deps / error fixtures) so the W-DEPRECATED-driven Migration 4 fails-closed on them. Bulk-migrating would override a deliberate exclusion + strip deprecation-test coverage. "Eliminate fully" = the READ surfaces (examples + docs, DONE). Samples retain `server function` BY DESIGN. No D4b dispatch.
- 2026-06-11 S180: **D3.1 (tool-fix Gap A+B) dispatched @ `7f641010` — IN FLIGHT.** Last dispatch of the arc; close (D5) follows its landing.
- **D5 close plan:** D3.1 land+push · targeted final confirmation (read-surface grep + recompile key examples + full suite) · file error-msg follow-on gap (compiler suggestions still say `server function` — type-system.ts:8502 + SPEC saveOrder/logEdit/getUser depictions) · file g-sse-server-keyword gap (SSE DD filed at scrml-support/docs/deep-dives/sse-server-keyword-deferred-2026-06-11.md) · update g-server-keyword-drift → resolved-partial (read surfaces done; samples-left-by-design; SSE deferred) · VERIFIED.md re-flag (examples changed) · 8-step wrap.
