...
Current baseline (2026-05-22 **S119 CLOSE**). Full `bun test` **19,506 pass / 220 skip / 1 todo / 0 fail / 744 files / 59,650 expect**. **The M5-swap is landed** — A3 hoist synthesis → C1 FileAST assembler → C2 the `--parser=scrml-native` routing change all shipped; the native parser is routable end-to-end behind the opt-in flag. The C2 dual-pipeline canary measures **949/1000 corpus files (94.9%)** producing a native FileAST identical to the live pipeline; the remaining 51 are catalogued follow-ups.
