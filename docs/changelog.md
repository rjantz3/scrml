...
Current baseline (2026-05-07 S67 close): **9,241 pass / 54 skip / 1 todo / 0 fail** (full suite); **~8,470 pre-commit subset**. +151 net pass / +10 skip from S66 close; 0 regressions. **A1b Wave 3 partial** — B7 + B8 + B9 + B10 (full three-phase: catalog + walker + cycle detection) all SHIPPED at S67; B11 + B12 + B13 audited and ready. **A1b Wave 4 (B14-B17 engine wave) audited** end-to-end. **A1b Wave 5 (B18-B22 cross-cutting) audited** (bundled doc). **File-delta dispatch-landing pattern locked** in pa.md (supersedes worktree+cherry-pick + brief fast-forward experiment). **Primer §7 corrected** to canonical §51.0.F three target-only `rule=` forms. **§6.11 footnote** records type-shape correction per §55. **S67 v0.2.0 scope expansion ratified**: Insight 22 (`test-bind`) + Insight 23 (DD-Harel hierarchy + history + internal/external + parallel sugar; OQ-Harel-8 resolved to `<engine>`; Machine Cohesion sharpened) + Item C (temporal `<machine>` → `<engine>` migration via `<onTimeout>` element) + Item G (B-shakeable timeouts) — net ~57-94h add to v0.2.0. **Roadmap docs caught up**: master-list §0 + IMPLEMENTATION-ROADMAP §2.5 + §2.6 + new SCOPE-SUPPLEMENT-2026-05-07.md. Next: A5/A6 phases + B11-B22 dispatches.
