...
Current baseline (2026-05-18 **S102 CLOSE — v0.3.3 CUT** · ~25-commit session across PGO Phase 3 (P3.A regex collapse + P3.B detect-runtime-chunks fold + P3.C owner-stack + P3.B-followup hasResetExpr flag) + §41.14 formFor SPEC + formFor impl (recognition + AST expansion + 8 error codes + 58 tests) + runtime-perf SCOPING · PGO Phase 3 wave landed trucking-dispatch pipeline 2326ms → ~880ms median = **−62%** reduction · S94 hypothesis REFUTED (actual hot path was post-fn-name-mangle 58% + detect-runtime-chunks 33%, NOT emit-bindings + emit-reactive-wiring as anticipated) · formFor end-to-end verified compile with PE-default action= + CSRF + shape-dispatched inputs + auto-synth labels · S102 user direction "include vanilla-JS baseline in phase 1" for runtime-perf SCOPING · S102 process violation: PA's `rebuild-self-host-dist.ts` overwrite of May-11 dist files introduced 1 pre-existing-aggravated self-host bootstrap break; --no-verify push user-authorized per S88): pre-commit subset **12,718 pass / 88 skip / 1 todo / 1 fail / 663 files / 43,030 expect** (the 1 fail is the locally-introduced self-host bootstrap issue; dist files gitignored so broken state is local-only). v0.3.3 follows v0.3.2 (S101 CLOSE).
