...
Current baseline (2026-05-19 **S108 CLOSE** — 20-commit landing arc: match block-form Tier 1 closed end-to-end (Phases 3+4) + Bug 5 P3 + SPEC §7.4.2 + Bug 1 floor + Bug 1 FULL fix in 3 waves + PGO Phase 3 C2 fold + formFor B5 L2 label-store + Bug 4 deep-dive + Bug 4 C-narrow + maps refresh + README current-state trim): pre-commit subset **13,304 pass / 88 skip / 1 todo / 0 fail / 690 files / 44,794 expect**; full `bun test compiler/tests/` **16,147 pass / 169 skip / 1 todo / 0 fail / 723 files / 47,209 expect**. Delta vs S107 close (15,930 / 714 / 46,845): **+217 pass / +9 files / +364 expect / 0 fail / 0 regressions**. **Headlines:** (1) **Match block-form Tier 1 case-analysis closed end-to-end** — Phase 3 codegen (new `emit-match.ts` consumer reusing the S78-factored `emit-variant-guard.ts` helper, ~430 LOC) + Phase 4 `:`-shorthand body codegen (parseExprToNode + synthesized logic > bare-expr AST routed through generateHtml interpolation case); was opaque HTML pass-through at S107 OPEN; now end-to-end functional for bare-body markup / self-closing / `:`-shorthand expressions / parenthesized payload bindings; 15 new tests. (2) **Bug 5 Phase 3 + SPEC §7.4.2** — constant-folding (Option γ) closes Bug 5 arc end-to-end; canonical adopter shape `const VERSION = "v0.3.0"` + `${VERSION}` now folds inline at compile time (zero placeholder + zero JS wiring + zero runtime cost); NEW `compiler/src/codegen/const-fold-env.ts` (~155 LOC, cached env via `partiallyEvaluateExpr`); SPEC §7.4.2 normative section (60 lines) authorizes the optimization; 14 unit tests. (3) **Bug 1 Tailwind FLOOR lint + FULL fix in 3 waves** — Floor (agent, 5 commits): W-TAILWIND-UNRECOGNIZED-CLASS info lint at SPEC §34 + §28 + §26.5 + 34 tests. Wave 1 (agent, 3 commits): grid/flex/aspect families + universal underscore-as-space + ratio + grid-track CSS functions (`repeat()` / `minmax()` / `fit-content()`) + 66 tests. Wave 2 (PA): transition/timing family + modern individual transform props + outline + `cubic-bezier()` / `steps()` + 26 tests. Wave 3 (PA): `transform-*` shorthand + 9 directional decl-transforms (`translate-x/y` / `scale-x/y` / `rotate-x/y/z` / `skew-x/y`) + 14 more function names in VALID_MATH_FUNCTIONS + 23 tests. ~25+ prefix families now emit real CSS; lint stops firing on the now-handled cases. (4) **formFor B5 L2 label-store consultation** — closes the wired-but-unconsumed `registerLabels` runtime; emit-form-for.ts buildFieldGroup now emits `${(typeof _scrml_label_for === "function" ? _scrml_label_for("Struct", "field") : "Mechanical Default")}` per `<label>` position; SPEC §41.14.7 amended with Codegen subsection. (5) **Bug 4 `?{` C-narrow + 530-line deep-dive** — was catastrophic EOF-cascade when bare `?{` appeared in markup-text body (scrml-about-scrml prose); deep-dive at `scrml-support/docs/deep-dives/bug-4-docs-mode-escape-2026-05-19.md` ratified Approach C-narrow per SPEC §3.1 + §8.1 conformance (11 prior-art systems analyzed; 372 workaround occurrences in adopter corpus; 86% of pages already used entity-escapes); implementation removed `?{` recognition from block-splitter.js markup-text loop; SPEC §4.17 amended with sibling locus-gating principle cross-ref; 8 dedicated tests + 3 existing block-splitter tests updated to C-narrow semantics. **Plus compile-time perf**: PGO Phase 3 C2 fold (agent, 3 commits) — skip `buildFunctionBodyRegistry` when `hasForStmt === false`; elide markup tag-test per-node when `hasChunkedMarkupTag === false`; mirrors S102 hasResetExpr + S106 hasEqualityExpr Option-2 pattern; 25 unit tests. **Plus housekeeping**: README current-state trim (52 → 30 lines; dropped stale v0.1/v0.2 framing) + maps refresh (8 maps refreshed via project-mapper agent at session-open).
