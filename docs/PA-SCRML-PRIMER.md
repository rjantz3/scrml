...
- **Computed-delay form** (§51.12.3.1) — `after=${expr}<unit>` accepts any non-negative-number expression. Applies to both `<onTimeout>` (engine) and legacy `<machine>` `.From after duration => .To` form. Static literals retain constant-fold path. **A5-5 codegen SHIPPED at S77 2026-05-10** for the engine `<onTimeout>` surface (msExpr arrow-fn in the timer-config table; runtime applies negative/NaN→0 clamp + Math.round); helper-level codegen surface (parseAfterDuration + emitDurationLiteral + parseMachineRules afterExpr branch + emit-machines.ts IIFE site + emit-logic.ts inline arm) IS in place for the legacy `<machine>` surface as well, BUT the legacy-machine body parser (block-splitter) splits `${...}` expressions into separate children, so the legacy form's computed-delay is gated on a follow-on body-parser fix (deferred — engine `<onTimeout>` is the S67-recommended surface).
