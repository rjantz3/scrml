...
| Q6 — `reset(@cell)` × lifecycle annotation? | **YES (a) symmetric reset.** Phase-0 STOP at impl dispatch (a587bef3011558e9f) surfaced a load-bearing prerequisite gap: SPEC §14.12.3 + §14.12.10 promise per-access tracking on Shape 1 reactive cells; impl tracker covered struct-field + fn-return loci ONLY. SPEC §6.8.3 + §14.12.10 cross-ref bullet LANDED S134 (option C of the 4-option panel: spec-only now, impl-after-prereq). **B-prereq SHIPPED S134 `fd58893e`** — Bug 19 HIGH RESOLVED; Shape 1 per-access tracker now functional (`type-system.ts` +671L; 25 new tests; full suite 21,676 → 21,701). **Q6-narrow now UNBLOCKED** (~10-20h queued dispatch — implements §6.8.3 ratified semantic on top of the now-functional tracker). |
...
2. **Q6 — reset×lifecycle SPEC amendment + impl.** Original brief: §6.8 + §14.12 normatively specify symmetric reset; type-system tracker listens for `_scrml_reset_*` writes; +5-8 regression tests. **S134 final status:** SPEC half LANDED (§6.8.3 NEW subsection + §14.12.10 cross-ref bullet, commit `e99f6763`); B-prereq Shape 1 tracker LANDED (`fd58893e`, Bug 19 RESOLVED); **Q6-narrow now UNBLOCKED** — queued ~10-20h dispatch via `scrml-js-codegen-engineer` (isolation:worktree) implements reset-awareness on top of the functional tracker.
