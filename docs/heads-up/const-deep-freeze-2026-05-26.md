...
| Q6 — `reset(@cell)` × lifecycle annotation? | **YES (a) symmetric reset.** Phase-0 STOP at impl dispatch (a587bef3011558e9f) surfaced a load-bearing prerequisite gap: SPEC §14.12.3 + §14.12.10 promise per-access tracking on Shape 1 reactive cells; impl tracker covers struct-field + fn-return loci ONLY. SPEC §6.8.3 + §14.12.10 cross-ref bullet LANDED S134 (this session — option C of the Phase-0 STOP's 4-option panel: spec-only now, impl deferred). **Impl DEFERRED** behind a new B-prereq dispatch (Shape 1 per-access tracker, ~20-30h) filed as Bug 19 HIGH in `docs/known-gaps.md`. Once B-prereq lands, Q6-narrow (~10-20h) becomes a queued unblocked dispatch. |
...
2. **Q6 — reset×lifecycle SPEC amendment + impl.** Original brief: §6.8 + §14.12 normatively specify symmetric reset; type-system tracker listens for `_scrml_reset_*` writes; +5-8 regression tests. **S134 update:** SPEC half LANDED (§6.8.3 NEW subsection + §14.12.10 cross-ref bullet); impl half DEFERRED behind Shape 1 tracker prerequisite (Bug 19 HIGH). Sequencing: B-prereq dispatch (~20-30h, fires Shape 1 lifecycle tracker on pre-transition reads) → Q6-narrow dispatch (~10-20h, reset semantics on top of the now-functional tracker). Both via `scrml-js-codegen-engineer` (isolation:worktree).
