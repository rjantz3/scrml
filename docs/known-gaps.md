...
### Bug 9 — Compiler-managed async transitive coloring (A9-class) — `deferred (DEFERRAL EMPIRICALLY CONFIRMED S138)`

When a client function calls a server function, the client function should be auto-async-and-awaited (per the "compiler owns the async wiring" pillar). Today the compiler doesn't fully thread this: `scheduling.ts::hasServerCallees` reads `route.functionName` (which is phantom — never set in `route-inference.ts:3018-3028`'s `routeMap.functions.set()` entry construction). The result: `serverFnNames` is ALWAYS empty; transitive client functions calling server functions never get `async`/`await` added, and the runtime silently runs the call as sync — returns a `Promise` where the call site expects the resolved value.

**S138 empirical investigation + rollback (the deferral was correct).** PA-direct attempted the L1 fix this session — adding `functionName: record.fnNode.name ?? null` to the routeMap.functions entry. L1 isolated regression tests (6 tests) PASSED — direct-caller case correctly emits `async function _scrml_load_*` with awaited server call. BUT R26 empirical sweep across 8 gauntlet dev sources (R24 ×4 + R25 ×4) revealed L1 fix REGRESSED 5 of 8 sources from `node --check` PASS to FAIL.

The unmasked downstream bug shape (across all 5 regressing sources): the CPS planner produces invalid JS by emitting `let _scrml__scrml_result_NN = await _scrml_fetch_<server>_<id>(...)` INSIDE a `Promise.all([...])` array literal. This is the canonical `let X = call() !{handler}` (failable call with error-effect handler) shape — the CPS planner shouldn't parallelize a failable-call-with-handler into a Promise.all element. Pre-L1, the containing client function wasn't async, so the `Promise.all` parallelization wasn't triggered + the `let !{handler}` shape stayed sequential. Post-L1, the function becomes async + the CPS planner triggers parallelization + the broken `let` shape lands inside the array literal.

This empirically confirms the original deferred classification: L1 alone is a blind-patch that regresses on real adopter source. The 3-layer fix is structurally required — L1 (functionName population) + L2-`shape-recognition` (CPS planner detects "this is a `let X = failable() !{handler}` shape; do NOT parallelize") + L3 (transitive async coloring across client fn graphs).

**Empirical sweep results (S138):**

| Source | Pre-L1 `node --check` | Post-L1 attempted `node --check` |
|---|---|---|
| r24/dev-1-react | PASS | FAIL (let-in-Promise-all) |
| r24/dev-2-go | PASS (0 async fns) | PASS (0 async fns) |
| r24/dev-3-svelte | PASS | FAIL (let-in-Promise-all) |
| r24/dev-4-pascal | FAIL (pre-existing) | FAIL (pre-existing) |
| r25/dev-1-react | PASS | FAIL (let-in-Promise-all) |
| r25/dev-2-elixir | PASS | FAIL (let-in-Promise-all) |
| r25/dev-3-svelte | PASS | FAIL (let-in-Promise-all) |
| r25/dev-4-pascal | PASS | PASS |

L1 fix reverted at HEAD; Bug 9 remains deferred. The unmasked CPS-planner shape bug (a `let X = failable() !{handler}` getting parallelized into a `Promise.all` array) is filed as Bug 55 below (NEW HIGH; class-level — affects 5 of 8 gauntlet sources).
...
- **Status:** DEFERRED to A9-class compiler-managed-async work per pa.md Rule 3 (3-layer fix; L3 = NEW transitive async-coloring subsystem; not blind-patched). Filed S126 + carried forward S127-S130. **S138 empirical confirmation:** the deferral was correct — L1 alone causes empirical regressions on 5 of 8 real adopter sources (CPS planner shape gap unmasked). Bug 9 stays deferred; integrated 3-layer fix queued for v0.7+ post-M6. Bug 55 (the unmasked CPS-planner shape bug) is the prerequisite L2 work for Bug 9's L1 to land safely.

---

### Bug 55 — CPS planner emits `let X = failable() !{handler}` inside `Promise.all([...])` array literal — `NEW S138 HIGH` (surfaced by Bug 9 L1 attempt)

When a client function contains a failable server call with an inline error handler (`let X = serverFn(args) !{| .Variant -> {...}}`) AND the function is `async` AND the CPS planner triggers parallelization, the planner emits the `let X = await serverFn(...)` + handler block as an ARRAY ELEMENT inside `await Promise.all([...])` — JS syntax error (a `let` statement isn't a valid array-literal element).

**Class scope:** 5 of 8 gauntlet dev sources (R24 ×3 + R25 ×2) regressed from `node --check` PASS to FAIL when the L1 Bug-9 attempted fix made these functions async. The fact that it's silent pre-L1 (because the containing function isn't async, so parallelization doesn't trigger) shouldn't be the canonical-correct state — the shape is wrong regardless. Adopter-visible would surface as soon as the containing function becomes async for ANY reason.

- **Reproducer (S138 emergent from gauntlet R24/R25 sources):** any `async function clientWrapper() { let X = failableCall() !{ | .Variant -> { ... } }; ... }` shape. Once L1 of Bug 9 (or any equivalent path that makes the wrapper async) is in place, the CPS planner triggers parallelization and emits the broken shape.
- **Current behavior:** `let _scrml__scrml_result_NN = await _scrml_fetch_<server>_<id>(...)` lands inside a `Promise.all([...])` array literal → SyntaxError.
- **Expected behavior:** the CPS planner should detect "this is a failable call with handler" and either NOT parallelize the let-stmt OR emit it via a different shape that's syntactically valid (e.g., outside the Promise.all batching).
- **Suggested fix scope:** CPS planner shape-recognition — detect the `let X = call() !{handler}` shape in the parallelization candidate list; route it through a non-parallel path. Likely in `compiler/src/codegen/cps-batch-planner.ts` or sibling.
- **Cross-refs:** Bug 9 L1 attempt empirical sweep (S138; this entry was the surfaced unmask); pa.md S138 R26 doctrine reverse-direction (sibling-fix-unmask check banked at scrml-support `dbb47c3`); SPEC §13.2 + §19.9.9 multi-batch CPS.
