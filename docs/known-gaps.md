...
**Gate status (S142):** `validateEmit` is wired with `--validate-emit` / `--no-validate-emit` CLI flags (compile/build/dev) + SPEC §2.2.1 operational-escape note, but the **default STAYS OFF** — the always-on flip is HELD on the residuals below. Full adopter + examples corpus now emits 0 invalid-JS artifacts under the gate; the within-node parity canary regression the fix-wave caused was investigated (BENIGN — LIVE moved malformed→correct) + the allowlist rebumped (`5be0a502`).

**§GATE-FOUND-RESIDUALS (S142, gate-flip blockers — close these → flip `validateEmit` default ON, a one-line change):** 3 forced-gate-ON failures remain, all INTERNAL (zero adopter-source impact; PA-confirmed the 2 self-host fire at the exact reported byte offsets):
- **self-host meta-checker** (`stdlib/compiler/meta-checker.scrml`, byte 10606) — multi-line ternary in a `const` initializer breaks `collectExpr` (MED — multi-line-ternary-in-const-init is an adopter-reachable JS shape).
- **self-host module-resolver** (`stdlib/compiler/module-resolver.scrml`, byte 4328) — escaped-backtick template + `not`-in-template-string mangling (LOW — niche).
- **nested `!{}`** (R25-Bug-49 §5) — arm handler captured as a flat token-string; no nested guarded-expr retention. Needs parser-level nested-error-effect-block retention OR codegen-time handler re-parse (MED — language-feature gap).

These are NOT in the §0 HIGH/MED/LOW counts above (tracked here as flip-prerequisites, the same way C10/C11 were before they were classified). Severities provisional pending triage.
