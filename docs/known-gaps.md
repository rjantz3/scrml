...
### Bug 71 — top-level derived `const <x> = match @cell {...}` (a derived STATE cell whose RHS is a value-return match) is NOT exhaustiveness-checked — `NEW S157; MED; DEFERRED` (PRE-EXISTING; surfaced by the Bug 67 dispatch via stash-baseline, NOT a Bug 67 regression)
