...
### Bug 62 — engine `.advance(...)` inside an `<each>`-render event handler → `E-CODEGEN-INVALID-JS` (raw `@` sigil leaks) — `NEW S155; HIGH; DEFERRED` (#14 batch-3-surfaced; PRE-EXISTING; affects state-plane `.advance` too)
