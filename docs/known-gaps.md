...
### Bug 57 — `<each>` Tier-1 iteration: `_scrml_reconcile_list` tree-shaken out of the runtime bundle — `NEW S140; HIGH; DISPATCHED S140` (PA-verified)
...
### Bug 58 — `formFor` validity surface never emitted (synth compound state-decl spliced into markup-children) — `NEW S140; HIGH; DISPATCHED S140` (PA-verified)
...
### Bug 59 — `tableFor` per-row checkbox onchange references undefined free var `evt` (Bug-50-class residual) — `NEW S140; HIGH; DISPATCHED S140` (PA-verified)
