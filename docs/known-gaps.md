...
### Bug 72 — nested `<each>` INSIDE a Tier-0 `${for…lift}` body → the nested-each `@.` is not codegen-rewritten in the lift-embedded position → `E-CODEGEN-INVALID-JS` — `NEW S157; MED; DEFERRED` (PRE-EXISTING; surfaced by the Bug 70 fix agent, stash-baseline-verified)
