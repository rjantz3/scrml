...
Surfaced by (d)-A batch 3 (`8f799c78`, S156). `tableFor`'s walker (`type-system.ts` `_processTableForNode` ~13263) has the SAME `asIs` leading-token fallback the batch-3 schemaFor fix just closed — a subset-refined enum field reaches it stripped to the BASE enum, so a `tableFor`-rendered column for that field would treat the value as the full enum (not the subset). The fix is the same shape as batch 3 (recover the subset PredicatedType before classify). Out of batch-3 scope (brief scoped to schemaFor). **Disposition — FOLDED into the (d)-A arc (user S156: "fold Bug 69 in too") as a post-batch-4 follow-on (batch 5): sequenced AFTER batch 4 lands, NOT parallel — batch 4 also touches `type-system.ts`, so a parallel dispatch would conflict at file-delta.** LOW-MED — tableFor display is the affected surface; correctness of the underlying value is unaffected (the subset is enforced at the type/match/schema loci). Cross-ref §41.16.6 / §53.15.
