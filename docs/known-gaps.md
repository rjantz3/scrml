...
| **R28-1b** | HIGH (suspected) | OPEN — needs runtime confirm | the block-form `<match on=…>` dispatcher is emitted at MODULE scope, not inside the `<each>` per-item factory → a match-on-loop-var inside `<each>` resolves to module scope (silent WRONG per-item value) for BOTH `@.field` (now lowered by R28-1) AND the `alias.field` workaround | Pre-existing, surfaced by the R28-1 agent's codegen analysis (dev-2: silently resolved to an unrelated formFor-synth `article` cell). "node --check clean ≠ correct" class (silent-wrong-output). Fix: emit the match dispatcher inside the each's keyed per-item factory. Needs happy-dom runtime confirmation before fix-dispatch (R26 reverse-direction). Separate follow-up dispatch. |
