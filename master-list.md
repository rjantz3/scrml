...
**In flight:** **scrml v0.2.0** breaking-change rewrite (~297-454h focused engineering — INCREASED by S67 ratified extensions: DD-Harel hierarchy + Item C temporal surface migration + Item G B-shakeable timeouts + Insight 22 test-bind, ~57-94h additional. Was ~240-360h post-S66 self-host deferral; was ~280-440h originally). See **§0 v0.2.0 Migration Status** below for live progress dashboard. Spec target frozen post-Stage-0b (S58) **as base**; S67 ratified extensions add to the spec target via Insight 22 + Insight 23 in `scrml-support/design-insights.md`. Phase A1+ implementation in progress; A1b foundational wave 1 (B1-B5) COMPLETE; A1b Wave 3 partial (B7+B8+B10 SHIPPED at S67; B9 SHIPPED at S67).
...
| **A7 — S67 ratified engine + temporal extensions** | DD-Harel hierarchy (`<engine>` nested per OQ-Harel-8 resolution; Machine Cohesion sharpened) + history attribute (tree-shakeable synth cell) + `internal:rule` prefix + `parallel` attribute sugar + Item C temporal surface migration (`<onTimeout>` element on `<engine>`) + computed-delay relaxation + Item G B-shakeable timeouts (event-timeout watchdog + named multi-timer) | **~50-80h** | ⏸️ pending; spec amendments queued | **S67 RATIFIED via master-PA debate verdicts (Insight 22 + Insight 23 at `scrml-support/design-insights.md`).** Item C audit on file at `docs/audits/item-c-temporal-engine-rule-migration-rule4-audit-2026-05-07.md` — recommends `<onTimeout>` structural element (Pillar-5-compliant + tooling-uniformity-compliant per S67 user observation). Sub-steps: (1) SPEC §51 amendments per the four DD-Harel grammar decisions + §51.0.M new + §51.12 cross-ref + Machine Cohesion footnote; (2) parser support for nested `<engine>`, `history` attr, `internal:rule` prefix, `parallel` attr, `<onTimeout>` element; (3) typer: variant validation across nested engines, history-cell synthesis, onTimeout `to=` variant check; (4) codegen: hierarchy desugar (~0 bytes net), history-cell tree-shake (~30-80 bytes per history-bearing parent), onTimeout lowering (rides existing `_scrml_machine_arm_timer` runtime); (5) computed-delay relaxation (parser/lower); (6) Item G timer-extension wiring (B-shakeable per S67 user-direction signal #2); (7) tests + samples. Total ~50-80h. Class A throughout per master-PA cost-lens. |
