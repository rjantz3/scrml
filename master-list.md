...
**Last updated:** 2026-05-11 (S81 — **F.1+F.2 adopter-override surface (cors-max-age + channel-reconnect) SHIPPED · strict self-host rebuild gate ACTIVE (honors "null/undefined never compile" directive) · Phase A10 deferred items CLOSED (TS body-walk + payload-binding scope injection) · SPEC-INDEX line-range regen (62 rows + persistent `scripts/regen-spec-index.ts`) · D3 pure-fn detection in monotonicity classifier SHIPPED (reduces idempotency-key over-emission) · D1 export-synth idempotent modifier propagation SHIPPED · OQ-2 imperative debounce/throttle keyword-form RETIRED (zero adopter footprint; net -87 LOC) · 7 commits / +42 net tests / 0 regressions / full-suite 11,181 pass · 1 day**)

**Tests (current — S81 close, pre-commit subset):** **10,433 pass / 66 skip / 1 todo / 0 FAIL** (507 files; `bun test compiler/tests/{unit,integration,conformance} --bail` ~12s). **Full suite (S81 close, `bun run test` incl. browser/lsp/commands/self-host):** **11,181 pass / 77 skip / 1 todo / 0 FAIL** (535 files). +1 new test file (`engine-body-typecheck-a10-followon.test.js`); +42 net new tests vs S80. Pre-commit hook firing every commit (installed S79); cross-machine sync clean.
